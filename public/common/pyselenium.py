...
from  public.common.public_functions import  cost_time
success = "SUCCESS   "
fail = "FAIL   "
logger = Log()
...
class PySelenium(object):
    ...
    def click(self, css):
        ...
        try:
            ...
            #TODO  截图 self.take_screenshot()
            raise

    def click_force_js(self,css,force=False):
        """
        当元素不可点击时，使用el.click()不生效使用暴力点击
        :param css:
        :type css:
        :param force: 是否强制点击
        :type force:
        ...
        try:
            ...
            def run_func():
                ...
                el =self.get_element(css)
                if not force:
                    self.origin_driver.execute_script("arguments[0].click()",el)
                else:
                    self.origin_driver.execute_script("arguments[0].click({force: true})",el)
                self.my_print("{0} Clicked element: <{1}>, Spend {2} seconds".format(success, css,run_func()))
        except Exception :
            self.my_print("{0} Unable to click element: <{1}>".format(fail, css))
    ...
    def move_to_element(self, css):
        ...
        try:
            ...
            raise
    def move_to_postion(self,xoffset,yoffset):
        """
         move to postion
        :param css:
        :type css:
    ...
    def get_attribute(self, css, attribute):
        ...
        driver.get_attribute("id->su","name")
    ...
    def select_checkbox(self, css, kw):
        '''
        Usage:
        driver.select_checkbox('id->kw','index:1')
        :param css: id->kw
        :param kw: index:0
        :return:
        s.select_by_index(1)  # 选择第二项选项：o1
        s.select_by_value("o2")  # 选择value="o2"的项
        s.select_by_visible_text("o3")  # 选择text="o3"的值，即在下拉时我们可以看到的文本
        '''

        t1 = time.time()
        try:
            s = Select(self.get_element(css))
            if ":" not in kw:
                raise NameError("Positioning syntax errors, lack of ':'.")
            k = css.split(":")[0].strip()
            v = css.split(":")[1].strip()
            if k == "index":
                ...
                raise NameError(
                    "Please enter the correct method,'index:1','value:12', 'visible_text:v'.")
            self.my_print(
                " {0}, Element {1} : method: {2}, Spend {3} seconds".format(success, css, kw, time.time() - t1))
        except Exception:
            self.my_print("{0} Unable to use select_checkbox method: element:{1}, method: {2} Spend {3} seconds".format
                          (fail,
                           css, kw,
                           time.time() - t1))
            raise
